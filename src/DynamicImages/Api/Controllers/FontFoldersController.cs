...
/// <summary>Folder CRUD for the Fonts tree, as <see cref="TemplateFoldersController"/>. Moves are on <see cref="FontTreeController"/>.</summary>
public class FontFoldersController(IFontFolderService folderService) : DynamicImagesControllerBase
{
    [HttpPost("fonts/folders")]
    [ProducesResponseType(typeof(FontFolderResponse), StatusCodes.Status201Created)]
    ...
    public IActionResult Create([FromBody] CreateTemplateFolderRequest request)
    {
        ...
        return result.Outcome == TreeOperationOutcome.Success
            ? Created($"fonts/folders/{result.Folder!.Key}", FontFolderResponse.From(result.Folder))
            : Failure(result.Outcome, request.Key ?? Guid.Empty);
    }

    [HttpGet("fonts/folders/{key:guid}")]
    [ProducesResponseType(typeof(FontFolderResponse), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public IActionResult Get(Guid key)
        => folderService.Get(key) is { } folder ? Ok(FontFolderResponse.From(folder)) : Failure(TreeOperationOutcome.NotFound, key);

    [HttpPut("fonts/folders/{key:guid}")]
    [ProducesResponseType(typeof(FontFolderResponse), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public IActionResult Update(Guid key, [FromBody] UpdateTemplateFolderRequest request)
    {
        ...
        return result.Outcome == TreeOperationOutcome.Success
            ? Ok(FontFolderResponse.From(result.Folder!))
            : Failure(result.Outcome, key);
    }

    [HttpDelete("fonts/folders/{key:guid}")]
    ...
    private IActionResult Failure(TreeOperationOutcome outcome, Guid key) => outcome switch
    {
        ...
        TreeOperationOutcome.NotEmpty => Problem(title: "The folder is not empty",
            detail: "Move or delete the font families and folders inside it first.", statusCode: StatusCodes.Status409Conflict),
